...
class DDQNAgent:
    ...
    def train(self):
        ...
        # 教師信号を求める ← ！！Target Q-function を使う！！
        with chainer.using_config('train', False), \
                chainer.using_config('enable_backprop', False):
            ...
            q_max[indices] = F.max(self.q_target(values), axis=1).data
...
class Trainer:
    ...
    def __init__(self, env, agent: DDQNAgent):
        self.env = env
        self.agent = agent
    ...
    def run(self, max_episode):
        ...
        for episode in range(max_episode):
            ...
            # 5エピソードごとにTarget Q-function を更新
            if episode % 5 == 0:
...
def main():
    ...
    agent = DDQNAgent(optimizer, gamma=0.99)
